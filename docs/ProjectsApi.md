...
> Project ProjectsRetrieve(ctx, id).XAccountToken(xAccountToken).IncludeRemoteData(includeRemoteData).Execute()
...
func main() {
    ...
    configuration := openapiclient.NewConfiguration()
    api_client := openapiclient.NewAPIClient(configuration)
    resp, r, err := api_client.ProjectsApi.ProjectsRetrieve(context.Background(), id).XAccountToken(xAccountToken).IncludeRemoteData(includeRemoteData).Execute()
    if err != nil {
...
func main() {
    ...
    // response from `ProjectsUsersList`: PaginatedUserList
    fmt.Fprintf(os.Stdout, "Response from `ProjectsApi.ProjectsUsersList`: %v\n", resp)
}
```
...
Other parameters are passed through a pointer to a apiProjectsUsersListRequest struct via the builder pattern
